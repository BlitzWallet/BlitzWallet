...
import { getAllRefundableQuotes } from '../../../../../functions/spark/getDepositAdressTxIds';

export default function BitcoinSwapsPage() {
  ...
  useFocusEffect(
    useCallback(() => {
      async function loadFailedBitcoinPayments() {
        const quotes = await getAllRefundableQuotes();
        setSwapList(quotes);
      }
      loadFailedBitcoinPayments();
    }, []),
  );
  ...
  if (swapList === null) {
    return (
      <FullLoadingScreen
        ...
        text={'Loading failed Bitcoin swaps'}
      />
    );
  }
  if (!swapList?.length) {
    return (
      <FullLoadingScreen
        ...
        textStyles={styles.textStyles}
        text={'You have no saved failed Bitcoin swaps'}
      />
    );
  }
  return (
