...
const getFlashnetClient = mnemonic => {
  const hash = getMnemonicHash(mnemonic);
...
// ============================================
// FLASHNET POOL QUERIES
// ============================================

export const listFlashnetPools = async ({ mnemonic, filters = {} }) => {
  try {
    const runtime = await selectSparkRuntime(mnemonic);
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.listFlashnetPools,
        { mnemonic, filters },
      );
      return validateWebViewResponse(
        response,
        'Not able to list Flashnet pools',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      // getAssetIdsFromFlashnetPools(mnemonic);
...
export const getAssetIdsFromFlashnetPools = async mnemonic => {
  try {
    console.log('=== QUERYING FLASHNET POOLS FOR ASSET IDs ===\n');

    const poolsResult = await listFlashnetPools({ mnemonic });

    if (!poolsResult.didWork || !poolsResult.pools) {
      console.error('Failed to get pools:', poolsResult.error);
      return;
    }

    console.log(`Found ${poolsResult.pools.length} pools\n`);

    // Store unique asset IDs we find
    const assetIds = new Set();
    let btcUsdbPool = null;

    poolsResult.pools.forEach((pool, index) => {
      console.log(`\nPool ${index + 1}:`);
      console.log('Pool ID:', pool.poolId || pool.lpIdentityPublicKey);
      console.log('Asset A Public Key:', pool.assetA);
      console.log('Asset B Public Key:', pool.assetB);
      console.log('Active:', pool.active);

      // Collect all asset IDs
      if (pool.assetA) assetIds.add(pool.assetA);
      if (pool.assetB) assetIds.add(pool.assetB);

      // Try to identify BTC/USDB pool
      // You might need to match by looking at pool names or testing
      const poolName = pool.name || pool.description || '';
      if (
        poolName.toLowerCase().includes('usdb') ||
        poolName.toLowerCase().includes('btc')
      ) {
        console.log('  >>> POTENTIAL BTC/USDB POOL <<<');
        btcUsdbPool = pool;
      }
    ...
    console.log('\n=== ALL UNIQUE ASSET IDs (Public Keys) ===');
    Array.from(assetIds).forEach((assetId, i) => {
      console.log(`${i + 1}. ${assetId}`);
    });
    ...
    if (btcUsdbPool) {
      ...
      console.log(
        'BTC_USDB_POOL_ID:',
        btcUsdbPool.poolId || btcUsdbPool.lpIdentityPublicKey,
      );
      console.log('BITCOIN_ASSET_PUBKEY:', btcUsdbPool.assetA);
      console.log('USDB_ASSET_PUBKEY:', btcUsdbPool.assetB);
      console.log('\nOr swap A/B if these seem reversed');

      return {
        poolId: btcUsdbPool.poolId || btcUsdbPool.lpIdentityPublicKey,
        bitcoinAssetId: btcUsdbPool.assetA,
        usdbAssetId: btcUsdbPool.assetB,
      };
    } else {
      console.log('\n⚠️ Could not auto-identify BTC/USDB pool');
      console.log('Please manually inspect the pools above');
    }

    return poolsResult.pools;
  } catch (err) {
    console.error('Get asset IDs from pools error:', err);
  }
};

export const getFlashnetPoolDetails = async ({ poolId, mnemonic }) => {
  try {
    const runtime = await selectSparkRuntime(mnemonic);
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.getFlashnetPoolDetails,
        { mnemonic, poolId },
      );
      return validateWebViewResponse(response, 'Not able to get pool details');
    } else {
      const client = getFlashnetClient(mnemonic);
      const pool = await client.getPoolDetails({ poolId });
...
// ============================================
// BITCOIN <-> USDB SWAP SIMULATIONS
// ============================================

export const simulateBitcoinToUSDB = async ({
  poolId,
  amountSats,
  bitcoinPubkey,
  usdbPubkey,
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.simulateBitcoinToUSDB,
        { mnemonic, poolId, amountSats, bitcoinPubkey, usdbPubkey },
      );
      return validateWebViewResponse(
        response,
        'Not able to simulate Bitcoin to USDB swap',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const simulation = await client.simulateBitcoinToUSDB({
        poolId,
        amountSats,
        bitcoinPubkey,
        usdbPubkey,
      });
      return { didWork: true, simulation };
    }
  } catch (err) {
    console.log('Simulate Bitcoin to USDB error', err);
    return { didWork: false, error: err.message };
  }
};

export const simulateUSDBToBitcoin = async ({
  poolId,
  amountUSDB,
  usdbPubkey,
  bitcoinPubkey,
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.simulateUSDBToBitcoin,
        { mnemonic, poolId, amountUSDB, usdbPubkey, bitcoinPubkey },
      );
      return validateWebViewResponse(
        response,
        'Not able to simulate USDB to Bitcoin swap',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const simulation = await client.simulateUSDBToBitcoin({
        poolId,
        amountUSDB,
        usdbPubkey,
        bitcoinPubkey,
    ...
    console.log('Simulate USDB to Bitcoin error', err);
    return { didWork: false, error: err.message };
  }
};

// ============================================
// BITCOIN <-> USDB SWAP EXECUTION
// ============================================
...
export const swapBitcoinToUSDB = async ({
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.swapBitcoinToUSDB,
        {
          mnemonic,
          poolId,
          amountSats,
      ...
      return validateWebViewResponse(
        response,
        'Not able to swap Bitcoin to USDB',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const result = await client.swapBitcoinToUSDB({
        poolId,
        amountSats,
        bitcoinPubkey,
        usdbPubkey,
      ...
      return { didWork: true, swap: result.swap };
    }
  } catch (err) {
...
export const swapUSDBToBitcoin = async ({
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.swapUSDBToBitcoin,
        {
          mnemonic,
          poolId,
          amountUSDB,
      ...
      return validateWebViewResponse(
        response,
        'Not able to swap USDB to Bitcoin',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const result = await client.swapUSDBToBitcoin({
        poolId,
        amountUSDB,
        usdbPubkey,
        bitcoinPubkey,
...
export const estimateUSDBForLightning = async ({
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.estimateUSDBForLightning,
        { mnemonic, invoice, poolId, usdbPubkey, bitcoinPubkey },
      );
      return validateWebViewResponse(
        response,
        'Not able to estimate USDB for Lightning payment',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const result = await client.estimateUSDBForLightningPayment({
        invoice,
        poolId,
        usdbPubkey,
        bitcoinPubkey,
      ...
      return { didWork: true, estimate: result.estimate };
    }
  } catch (err) {
...
export const payLightningInvoiceWithUSDB = async ({
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.payLightningWithUSDB,
        {
          mnemonic,
          invoice,
          poolId,
      ...
      return validateWebViewResponse(
        response,
        'Not able to pay Lightning invoice with USDB',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const result = await client.payLightningWithUSDB({
        invoice,
        poolId,
        usdbPubkey,
        bitcoinPubkey,
      ...
      return { didWork: true, result: result.result };
    }
  } catch (err) {
...
export const getUserSwapHistory = async ({ limit = 50, mnemonic }) => {
  try {
    const runtime = await selectSparkRuntime(mnemonic);
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
      ...
      return validateWebViewResponse(
        response,
        'Not able to get user swap history',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
...
export const getPoolSwapHistory = async ({ poolId, limit = 100, mnemonic }) => {
  try {
    const runtime = await selectSparkRuntime(mnemonic);
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.getPoolSwapHistory,
        { mnemonic, poolId, limit },
      );
      return validateWebViewResponse(
        response,
        'Not able to get pool swap history',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
    ...
    console.log('Get pool swap history error', err);
    return { didWork: false, error: err.message };
  }
};
...
export const executeRouteSwap = async ({
  hops,
  amountIn,
  ...
  try {
    ...
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
        OPERATION_TYPES.executeRouteSwap,
        { mnemonic, hops, amountIn, maxRouteSlippageBps },
      );
      return validateWebViewResponse(
        response,
        'Not able to execute route swap',
      );
    } else {
      const client = getFlashnetClient(mnemonic);
      const result = await client.executeRouteSwap({
        hops,
        amountIn,
        maxRouteSlippageBps,
      });
      ...
      return { didWork: true, routeSwap: result.routeSwap };
    }
  } catch (err) {
    console.log('Execute route swap error', err);

    // Check for refund in error response
    if (err.refundTransferId) {
      return {
        didWork: false,
        error: err.error || 'Route swap failed',
        errorType: 'ROUTE_FAILED',
        refund: {
          transferId: err.refundTransferId,
          amount: err.refundedAmount,
          assetPublicKey: err.refundedAssetPublicKey,
        },
      };
    }
...
export const requestClawback = async ({
  sparkTransferId,
  poolId,
  mnemonic,
}) => {
  try {
    ...
    if (runtime === 'webview') {
      ...
      const client = getFlashnetClient(mnemonic);
      const result = await client.requestClawback({
        sparkTransferId,
        poolId,
      });
      ...
      if (result.accepted) {
        ...
        return {
          didWork: false,
          error: result.error || 'Clawback request rejected',
          errorType: 'CLAWBACK_REJECTED',
        };
      }
    }
  } catch (err) {
    console.log('Request clawback error', err);
    return { didWork: false, error: err.message, errorType: 'CLAWBACK_FAILED' };
  }
};

export const checkClawbackStatus = async ({ internalRequestId, mnemonic }) => {
  try {
    const runtime = await selectSparkRuntime(mnemonic);
    if (runtime === 'webview') {
      const response = await sendWebViewRequestGlobal(
